...
final class StatusBarAccessibilityProvider {
    ...
    private func createSpendingAccessibilityInfo() -> String {
        ...
        let spendingText = "\(currencyData.selectedSymbol)\(userSpending.formatted(.number.precision(.fractionLength(2))))"
        ...
        return "\(statusText). Current spending: \(spendingText) of \(limitText) limit. \(Int(percentage)) percent used."
    }
    ...
    private func determineStatusText(for percentage: Double) -> String {
        ...
        case 80..<100:
            return "High usage, approaching limit"
        default:
            return "Over limit"
        }
    }
