...
struct CursorDataTransformer {
    ...
    static func transformInvoice(from response: CursorInvoiceResponse, month: Int, year: Int) -> ProviderMonthlyInvoice {
    ...
    static func transformUsageData(from response: CursorUsageResponse) throws -> ProviderUsageData {
        ...
        logger.info("Successfully transformed Cursor usage: \(primaryUsage.numRequests)/\(primaryUsage.maxRequestUsage ?? 0) requests")
