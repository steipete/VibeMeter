...
final class DataFetchingService {
    ...
    func fetchMultipleProviderData(for providers: [ServiceProvider]) async -> [ServiceProvider: Result<ProviderDataResult, Error>] {
...
enum DataFetchingError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .providerDisabled(let provider):
            return "Provider \(provider.displayName) is disabled"
        case .noAuthToken(let provider):
            return "No authentication token found for \(provider.displayName)"
        }
    }
}