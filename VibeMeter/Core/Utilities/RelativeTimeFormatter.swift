...
public struct RelativeTimeFormatter {
    ...
    nonisolated(unsafe) private static let shortFormatter: RelativeDateTimeFormatter = {
        let formatter = RelativeDateTimeFormatter()
...
extension RelativeTimeFormatter {
    /// SwiftUI color for data freshness
    public static func swiftUIFreshnessColor(for date: Date) -> Color {
...
public struct RelativeTimestampView: View {
    ...
    public var body: some View {
        Text(RelativeTimeFormatter.string(from: date, style: style))
            .foregroundStyle(showFreshnessColor ? RelativeTimeFormatter.swiftUIFreshnessColor(for: date) : .secondary)
            .onAppear {
                startTimer()
            }
            .onDisappear {
...
extension RelativeTimeFormatter {
    ...
    public static var sampleDates: [(String, Date)] {
        let now = Date()
        return [
            ...
            ("Last week", now.addingTimeInterval(-604800))
        ]
    }
}
#endif