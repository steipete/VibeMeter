...
final class VocabularyValidator {
    ...
    struct ValidationResult {
        ...
        var summary: String {
            if isValid {
                "✅ Vocabulary valid: \(tokenCount) tokens, checksum: \(checksum.prefix(8))..."
            } else {
                "❌ Vocabulary invalid: \(errors.count) errors, \(warnings.count) warnings"
            }
        }
    }
    ...
    enum ValidationError: Error {
        ...
        var description: String {
            ...
            case let .duplicateToken(token, ids):
                "Duplicate token '\(token)' with IDs: \(ids)"
            case let .duplicateRank(rank, tokens):
                "Duplicate rank \(rank) for tokens: \(tokens)"
            case let .missingRequiredTokens(tokens):
                "Missing required tokens: \(tokens.joined(separator: ", "))"
    ...
    enum ValidationWarning {
        ...
        var description: String {
            switch self {
            case let .unusualTokenLength(token, length):
                "Unusual token length (\(length) bytes): '\(token)'"
            case let .gapInRanks(start, end):
                "Gap in ranks from \(start) to \(end)"
            case let .unusualSpecialToken(token):
                "Unusual special token: '\(token)'"
            case let .performanceImpact(message):
    ...
    private func validatePerformance(
        ...
        if bytePairRanks.count > 500_000 {
            warnings
                .append(.performanceImpact("Large vocabulary (\(bytePairRanks.count) tokens) may impact performance"))
        }

        // Many long tokens warning
        let longTokens = bytePairRanks.filter { $0.key.count > 20 }
        if longTokens.count > 10000 {
...
struct VocabularyStatistics {
    ...
    init(bytePairRanks: [Data: Int], specialTokens: [String: Int]) {
        ...
        let lengths = bytePairRanks.keys.map(\.count)
...
extension VocabularyValidator {
    ...
    func repair(vocabularyURL: URL, outputURL _: URL) async -> (success: Bool, changes: [String]) {
        var changes: [String] = []
