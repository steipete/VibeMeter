...
struct PerformanceBenchmarks {
    ...
    struct StringOperations {
        ...
        func menuBarStringFormattingPerformance() {
            // Given
            let amounts = stride(from: 0.0, through: 999_999.99, by: 100.0).map(\.self)
            let currencies = ["$", "€", "£", "¥"]
    ...
    struct MenuBarUpdates {
        ...
        func gaugeAnimationCalculations() {
            ...
            let progressValues = stride(from: 0.0, through: 1.0, by: 0.001).map(\.self)
            ...
            // Then
            print("Calculated \(progressValues.count + 1_100_000) animation values in \(duration) seconds")
            #expect(duration < 5.0) // Should complete in under 5 seconds
        }
        ...
        func menuTextGenerationPerformance() {
            // Given
            let amounts = stride(from: 0.0, through: 999_999.99, by: 1000.0).map(\.self)
