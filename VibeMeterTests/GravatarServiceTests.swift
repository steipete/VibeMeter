...
final class GravatarServiceTests: XCTestCase {
    ...
    func testGravatarURL_ValidEmail_GeneratesCorrectURL() {
        ...
        let expectedHash = "973dfe463ec85785f5f95af5ba3906eedb2d931c24e69824a89ea65dba4e813b" // SHA256 of "test@example.com"
    ...
    func testGravatarURL_EmailWithWhitespace_TrimsAndLowercases() {
        ...
        XCTAssertEqual(result1?.absoluteString, result2?.absoluteString, "Should trim whitespace and convert to lowercase")
    }
    ...
    func testGravatarURL_CustomSize_DoublesForRetina() {
        ...
        XCTAssertTrue(result?.absoluteString.contains("s=\(expectedRetinaSize)") ?? false, "Should double size for retina display")
    }
    ...
    func testGravatarURL_DefaultSize_Uses40Points() {
        ...
        // Then
        XCTAssertNotNil(result)
        XCTAssertTrue(result?.absoluteString.contains("s=\(expectedRetinaSize)") ?? false, "Should use default size of 40 points (80 retina)")
    }
    ...
    func testSHA256Hashing_KnownInputs_GeneratesExpectedHashes() {
        ...
        let testCases = [
            ...
            ("admin@site.net", "5e884898da28047151d0e56f8dc6292773603d0d6aabbdd62a11ef721d1542d8") // SHA256 of "password" but this is "admin@site.net"
        ]
        ...
        for (email, expectedHashStart) in testCases {
            ...
            if let url = result?.absoluteString,
               ...
               let queryRange = url.range(of: "?") {
                ...
                XCTAssertTrue(extractedHash.allSatisfy { $0.isHexDigit }, "Hash should contain only hex digits for email: \(email)")
            } else {
    ...
    func testSHA256Hashing_DifferentEmails_GenerateDifferentHashes() {
        ...
        // Then
        XCTAssertNotEqual(url1?.absoluteString, url2?.absoluteString, "Different emails should generate different hashes/URLs")
    }
    ...
    func testUpdateAvatar_WithValidEmail_SetsCurrentAvatarURL() {
        ...
        XCTAssertTrue(sut.currentAvatarURL?.absoluteString.contains("gravatar.com") ?? false, "Should be a Gravatar URL")
    }
    ...
    func testGravatarURL_SpecialCharacters_HandlesCorrectly() {
        ...
        for email in emailsWithSpecialChars {
            ...
            XCTAssertTrue(result?.absoluteString.contains("gravatar.com") ?? false, "Should generate valid Gravatar URL for: \(email)")
        }
    }
    ...
    func testGravatarURL_LargeSize_HandlesCorrectly() {
        ...
        XCTAssertTrue(result?.absoluteString.contains("s=\(expectedRetinaSize)") ?? false, "Should handle large retina size")
    }
    ...
    func testGravatarService_IsObservable() {
        // Then
        XCTAssertTrue(type(of: sut) is any Observable.Type, "GravatarService should be Observable")
    }
