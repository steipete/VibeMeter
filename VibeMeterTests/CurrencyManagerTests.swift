...
final class CurrencyManagerTests: XCTestCase {
    ...
    func testAvailableCurrencies_ContainsCommonCurrencies() {
        ...
        // When
        let currencies = sut.availableCurrencies
        let currencyCodes = currencies.map { $0.0 }
    ...
    func testAvailableCurrencies_HasCorrectFormat() {
        ...
        for (code, name) in currencies {
            ...
            XCTAssertTrue(code.allSatisfy { $0.isUppercase }, "Currency code '\(code)' should be uppercase")
            
            // Name should contain currency symbol in parentheses
            XCTAssertTrue(name.contains("(") && name.contains(")"), "Currency name '\(name)' should contain symbol in parentheses")
    ...
    func testAvailableCurrencies_PrioritizesCommonCurrencies() {
        // When
        let currencies = sut.availableCurrencies
        let first10Codes = Array(currencies.prefix(10)).map { $0.0 }
    ...
    func testSystemCurrencyCode_ReturnsValidCode() {
        ...
        if let code = systemCode {
            ...
            XCTAssertTrue(code.allSatisfy { $0.isUppercase }, "System currency code should be uppercase")
