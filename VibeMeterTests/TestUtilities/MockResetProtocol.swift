...
extension MockResetProtocol {
    ...
    func resetCapturedParameter(_ keyPath: ReferenceWritableKeyPath<Self, (some Any)?>) {
        self[keyPath: keyPath] = nil
    }

    /// Helper to reset a captured parameters array.
    func resetCapturedArray(_ keyPath: ReferenceWritableKeyPath<Self, [some Any]>) {
...
class BaseMock: MockResetProtocol {
    ...
    func captureParameter(_ parameter: some Any, for methodName: String) {
        capturedParameters[methodName] = parameter
    }

    /// Gets a captured parameter for a method.
    func capturedParameter<T>(for methodName: String, as _: T.Type) -> T? {
