...
final class ProviderConnectionStatusTests: XCTestCase {
    ...
    func testCodable_RateLimited_WithoutDate_EncodesAndDecodes() throws {
        ...
        if case .rateLimited(let until) = decoded {
            XCTAssertNil(until)
        } else {
    ...
    func testCodable_RateLimited_WithDate_EncodesAndDecodes() throws {
        // Given
        let date = Date(timeIntervalSince1970: 1640995200) // Fixed date for testing
    ...
    func testUserFriendlyError_NetworkErrors() {
        ...
        for error in networkErrors {
            ...
            // Then
            XCTAssertEqual(status.description, "Connection failed", "Network error '\(error)' should be converted to user-friendly message")
        }
    }
    ...
    func testUserFriendlyError_AuthErrors() {
        ...
        for error in authErrors {
            ...
            // Then
            XCTAssertEqual(status.description, "Authentication failed", "Auth error '\(error)' should be converted to user-friendly message")
        }
    }
    ...
    func testUserFriendlyError_TimeoutErrors() {
        ...
        for error in timeoutErrors {
            ...
            // Then
            XCTAssertEqual(status.description, "Request timed out", "Timeout error '\(error)' should be converted to user-friendly message")
        }
    }
    ...
    func testUserFriendlyError_TeamNotFound() {
        ...
        for error in teamErrors {
            ...
            // Then
            XCTAssertEqual(status.description, "Team not found", "Team error '\(error)' should be converted to user-friendly message")
        }
    }
    ...
    func testUserFriendlyError_GenericError_Truncated() {
        ...
        // Then
        XCTAssertEqual(status.description, String(longError.prefix(50)), "Long error should be truncated to 50 characters")
    }
    ...
    func testConcurrentAccess_ThreadSafety() async {
        ...
        await withTaskGroup(of: Bool.self) { group in
            ...
            XCTAssertTrue(results.allSatisfy { $0 }, "All concurrent reads should succeed")
        }
    }
}