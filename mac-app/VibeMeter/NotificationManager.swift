...
final class NotificationManager {
    nonisolated(unsafe) private static var _shared: NotificationManagerProtocol = RealNotificationManager()
...
class RealNotificationManager: NSObject, NotificationManagerProtocol, UNUserNotificationCenterDelegate {
    ...
    func showWarningNotification(currentSpending: Double, limitAmount: Double, currencyCode: String) {
        ...
        content
            .body =
            "Heads up! Your Cursor spend (\(symbol)\(String(format: "%.2f", currentSpending))) is getting close to your \(symbol)\(String(format: "%.2f", limitAmount)) warning vibe!"
    ...
    func showUpperLimitNotification(currentSpending: Double, limitAmount: Double, currencyCode: String) {
        ...
        content
            .body =
            "Whoa! Your Cursor spend (\(symbol)\(String(format: "%.2f", currentSpending))) is hitting the \(symbol)\(String(format: "%.2f", limitAmount)) max vibe! Time to chill?"
        ...
        case .upper:
            if currentSpendingUSD < upperLimitUSD {
                if hasNotifiedForUpperLimitThisSession {
                    LoggingService.info(
                        "Spending dropped below upper limit. Resetting upper limit notification state for this session.",
                        category: .notification
                    )
    ...
    nonisolated func userNotificationCenter(
        ...
        LoggingService.debug(
            "User interacted with notification. Action: \(actionIdentifier == UNNotificationDefaultActionIdentifier ? "default" : actionIdentifier)",
            category: .notification
        )
