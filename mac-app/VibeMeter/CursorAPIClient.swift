...
final class CursorAPIClient { // This outer class now acts as the namespace/companion
    ...
    struct MonthlyInvoiceResponse: Codable, Equatable {
        ...
        var totalSpendingCents: Int {
            if let items = items {
                return items.reduce(0) { $0 + $1.cents }
            } else {
...
class RealCursorAPIClient: CursorAPIClientProtocol {
    ...
    private func performRequest<T: Decodable>(
        ...
        do {
            ...
            let errorMsg =
                "An unknown error occurred while processing API response for \(url.absoluteString): \(error.localizedDescription)"
        ...
        .MonthlyInvoiceResponse
    {
        ...
        let requestBody = CursorAPIClient.MonthlyInvoiceRequest(teamId: teamId, month: month, year: year, includeUsageEvents: true)
