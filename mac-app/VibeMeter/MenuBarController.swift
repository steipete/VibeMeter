...
class MenuBarController {
    ...
    private var warningLimit: Double? = 200.0 // Default
    private var upperLimit: Double? = 1000.0 // Default
    private var teamName: String?
    private var selectedCurrencySymbol: String = "$" // Default to USD
    private var exchangeRatesAvailable = true
    private var lastErrorMessage: String?
    private var teamIdFetchFailed = false
    ...
    @objc func settingsClicked() {
        ...
        if settingsWindow == nil || settingsWindow?.isVisible != true {
            let settingsView = SettingsView(settingsManager: dataCoordinator.settingsManager as! SettingsManager, dataCoordinator: dataCoordinator)
    ...
    // TODO: Add methods to be called by other managers to update UI state:
    // func updateLoginState(isLoggedIn: Bool, email: String?, teamName: String?)
    // func updateSpendingData(spending: Double?, warning: Double?, upper: Double?, currencySymbol: String)
    // func updateErrorState(message: String?, teamFetchFailed: Bool, ratesUnavailable: Bool)
}
