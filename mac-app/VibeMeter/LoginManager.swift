...
class LoginManager: NSObject {
    ...
    init(
        ...
        self._webViewFactory = webViewFactory
...
extension LoginManager: WKNavigationDelegate {
    ...
    @MainActor
    func webView(_ webViewParam: WKWebView, didCommit navigation: WKNavigation!) {
        ...
        LoggingService.debug(
            "LoginManager (as WKNavigationDelegate) - didCommit navigation to: \(currentWebView.url?.absoluteString ?? "unknown URL")",
            category: .login
        )
    ...
    func webView(_ webViewParam: WKWebView, didFinish navigation: WKNavigation!) {
        guard let currentWebView = webView else {
        ...
        LoggingService.debug(
            "LoginManager (as WKNavigationDelegate) - didFinish navigation to: \(currentWebView.url?.absoluteString ?? "unknown URL")",
            category: .login
        )
    ...
    private func checkForSessionCookie() {
        ...
        currentWebView.cookieStoreContract.getAllCookies { [weak self] cookies in
            ...
            if let sessionCookie = cookies.first(where: { $0.name == self.cookieName && $0.domain.contains(self.cookieDomain.dropFirst()) }) {
                LoggingService.info("Found \(self.cookieName) cookie, login complete!", category: .login)
                ...
                LoggingService.debug("Session cookie not yet available. Available cookies: \(cookies.map { "\($0.name)@\($0.domain)" }.joined(separator: ", "))", category: .login)
            }
        }
    }

    @MainActor
    func webView(_ webViewParam: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
