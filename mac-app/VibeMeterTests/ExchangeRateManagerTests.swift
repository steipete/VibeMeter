...
class ExchangeRateManagerTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        exchangeRateManager = ExchangeRateManager(userDefaults: testUserDefaults, session: mockURLSession)
    }
    ...
    func testGetCurrencySymbol() {
        ...
        XCTAssertEqual(ExchangeRateManager.getSymbol(for: "CAD"), "CA$")
        XCTAssertEqual(ExchangeRateManager.getSymbol(for: "AUD"), "A$")
        XCTAssertEqual(ExchangeRateManager.getSymbol(for: "CHF"), "CHF")
        XCTAssertEqual(ExchangeRateManager.getSymbol(for: "CNY"), "CN¥")
        XCTAssertEqual(ExchangeRateManager.getSymbol(for: "INR"), "₹")
        XCTAssertEqual(ExchangeRateManager.getSymbol(for: "PHP"), "₱")
        XCTAssertEqual(ExchangeRateManager.getSymbol(for: "XYZ"), "XYZ ") // Unknown currency
    }
    ...
    func testFetchExchangeRatesSuccessfully() async {
        ...
        XCTAssertNotNil(
            testUserDefaults.dictionary(forKey: ExchangeRateManager.Keys.cachedExchangeRates),
            "Rates should be cached."
        )
        XCTAssertNotNil(
            testUserDefaults.object(forKey: ExchangeRateManager.Keys.lastExchangeRateFetchTimestamp),
    ...
    func testFetchExchangeRatesWithNetworkError() async {
        ...
        let cachedRates = testUserDefaults.dictionary(forKey: ExchangeRateManager.Keys.cachedExchangeRates)
        XCTAssertNil(cachedRates, "Cache should not be updated on network error.")
    }

    func testFetchExchangeRatesWithInvalidJSON() async {
        let invalidJSONData = "{\"invalid_json\": \"not the expected structure\"}".data(using: .utf8)!
    ...
    func testGetRatesUsesCacheWhenValid() async {
        ...
        testUserDefaults.set(cachedRatesData, forKey: ExchangeRateManager.Keys.cachedExchangeRates)
        let recentTimestamp = Calendar.current.date(byAdding: .hour, value: -1, to: Date())!
        testUserDefaults.set(recentTimestamp, forKey: ExchangeRateManager.Keys.lastExchangeRateFetchTimestamp)
    ...
    func testGetRatesFetchesWhenCacheExpired() async {
        ...
        testUserDefaults.set(oldCachedRates, forKey: ExchangeRateManager.Keys.cachedExchangeRates)
        let expiredTimestamp = Calendar.current.date(byAdding: .hour, value: -25, to: Date())!
        testUserDefaults.set(expiredTimestamp, forKey: ExchangeRateManager.Keys.lastExchangeRateFetchTimestamp)
    ...
    func testGetRatesUsesFallbackWhenFetchFailsAndCacheInvalid() async {
        testUserDefaults.removeObject(forKey: ExchangeRateManager.Keys.cachedExchangeRates)
        testUserDefaults.removeObject(forKey: ExchangeRateManager.Keys.lastExchangeRateFetchTimestamp)
    ...
    func testGetRatesUsesFallbackWhenFetchReturnsNilAndCacheInvalid() async {
        ...
        mockURLSession.nextData = "{\"corrupt\":true}".data(using: .utf8)
        mockURLSession.nextResponse = HTTPURLResponse(
            url: URL(string: "https://api.frankfurter.app/latest?from=USD&to=EUR,GBP,JPY,CAD,AUD,CHF,CNY,INR,PHP")!,
            statusCode: 200,
            httpVersion: nil,
