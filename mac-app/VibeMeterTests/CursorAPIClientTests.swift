...
class CursorAPIClientTests: XCTestCase {
    ...
    private func createMockResponse(statusCode: Int, data: Data?) -> HTTPURLResponse? {
        HTTPURLResponse(
        ...
        do {
            ...
            if case let .networkError(underlyingError) = error {
                XCTAssertEqual(
                    ...
                    500,
                    "Underlying error should reflect status code or be specific type"
                )
            } else {
    ...
    func testFetchMonthlyInvoiceSuccessfully() async throws {
        ...
        XCTAssertFalse(decodedBody?.includeUsageEvents ?? true) // Check default
    }
