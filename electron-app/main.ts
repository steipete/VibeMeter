...
import { RendererSettings } from './src/types';
import { createLoginWindow, createSettingsWindow, settingsWindow } from './src/windows';
import { fetchData, fetchExchangeRatesIfNeeded } from './src/api';
import { checkLimitsAndNotify } from './src/notifications';
import { logOut } from './src/auth';
import { setTray, updateTray, buildContextMenu } from './src/tray';
...
function setLaunchAtLoginForTest(enable: boolean): void {
...
if (process.env.NODE_ENV === 'test') {
  const testExports: TestExports & 
    { 
      setLaunchAtLoginForTest: (enable: boolean) => void;
      quitAppForTest: () => void; // Add new function to type
    } = {
    checkLimitsAndNotify,
    logOut,
    updateTray,
